...
def patch_deepspeed_cuda_detection():
    ...
    def patched_check_output(cmd, *args, **kwargs):
        ...
        if isinstance(cmd, list) and len(cmd) >= 2 and "nvcc" in cmd[0] and "-V" in cmd:
            print(f"[PATCH] Intercepting nvcc subprocess call: {cmd}")
            ...
            if cuda_home:
                ...
                if match:
                    ...
                    print(f"[OK] Returning fake nvcc output for CUDA {major}.{minor}")
            ...
            print("[WARN] Could not detect CUDA version, returning fake CUDA 12.0")
        ...
        try:
            ...
            if "[WinError 6]" in str(e):
                print(f"[WARN] Subprocess handle error for command: {cmd}")
                print("[WARN] This is a known PyInstaller limitation on Windows")
                raise
            raise
    ...
    print("[OK] subprocess.check_output patched for DeepSpeed compatibility")
...
def setup_cuda_path():
    ...
    if cuda_path:
        if "CUDA_PATH" in os.environ and "CUDA_HOME" not in os.environ:
            ...
            print(f"[OK] Set CUDA_HOME to: {os.environ['CUDA_HOME']}")
        ...
        if os.path.exists(cuda_bin):
            ...
            if cuda_bin not in current_path:
                ...
                print(f"[OK] Added CUDA bin directory to PATH: {cuda_bin}")
            else:
                print(f"[OK] CUDA bin directory already in PATH: {cuda_bin}")
            ...
            for dll in key_dlls:
                ...
                if os.path.exists(dll_path):
                    print(f"[OK] Found CUDA DLL: {dll}")
                else:
                    print(f"[FAIL] Missing CUDA DLL: {dll}")
        else:
            print(f"[FAIL] CUDA bin directory not found: {cuda_bin}")
    else:
        print("[FAIL] CUDA_PATH environment variable not set.")
        print("Please ensure NVIDIA CUDA toolkit is installed and CUDA_PATH is set.")
        print(
            "Expected: CUDA_PATH=C:\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA\\v12.9"
        )
    ...
    for fallback_path in fallback_paths:
        if os.path.exists(fallback_path) and fallback_path not in current_path:
            ...
            print(f"[OK] Added fallback CUDA path: {fallback_path}")
...
# NOTE: setup_cuda_path() is intentionally disabled for smol builds.
# torch 2.5.1+cu121 ships with its own CUDA 12.1 DLLs bundled in _internal/torch/lib/.
# Adding system CUDA 12.9 to PATH causes DLL version conflicts.
# setup_cuda_path()
